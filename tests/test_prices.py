"""Reference prices: sourced dollars for `avs cost` — estimation, never gating.

With no prices configured every call is UNPRICED and the month's total is a
floor of $0.00 — honest, and useless to a founder asking "how much will a
typical month cost me?". The framework will not invent a price, so it ships
published list prices with a source and a date.

These tests pin the properties that make a shipped price table honest —
sourced, ceiling-not-guess, loud when stale, corrections surviving re-import
— and the ADR-032 boundary: pricing informs `avs cost` and the report, and
...
def test_a_range_resolves_upward_so_the_estimate_is_a_ceiling():
    """Under-counting is the failure that matters for a figure someone
    budgets around. Sonnet 5's introductory $2/$10 runs to 2026-08-31; the
    table carries the $3/$15 standard price and says why."""
...
def test_import_makes_the_month_answerable_in_dollars(tmp_path):
    """The end-to-end property: before the import the month prices at a
    $0.00 floor; after it, the same usage has a real dollar figure. A
    statement either way — nothing is ever refused (ADR-032)."""
    root = tmp_path / "w"
...
def test_an_unpriced_model_is_named_not_hidden(tmp_path):
    ...
    assert report.is_floor, "a total that hides an unpriced call understates"
