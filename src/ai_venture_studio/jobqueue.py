...
number of `avs worker` processes on the host claim them
...
CREATE TABLE IF NOT EXISTS jobs (
  ...
  kind TEXT NOT NULL,
  argv TEXT NOT NULL,          -- JSON list of CLI args after `avs`
...
def worker_loop(
    ...
    *,
    ...
    """Claim → run `avs <argv>` → record, forever (or max_jobs for
    ...
    while max_jobs is None or done < max_jobs:
        ...
        if runner is not None:
            ...
            proc = subprocess.run(  # noqa: S603 — fixed argv prefix
                [sys.executable, "-m", "ai_venture_studio.cli", *job.argv],
