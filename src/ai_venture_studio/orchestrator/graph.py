...
`hitl` via interrupt(); `avs resume <review-id> --decision ...`
...
from ai_venture_studio.tools import run_all
from ai_venture_studio.tools.base import render_for_context
...
def dor_gate_node(state: ReviewState, *, repo_dir: str) -> dict[str, Any]:
    ...
    from ai_venture_studio.policy import load_policy
...
def vote_node(
    ...
    from ai_venture_studio.review_gate import REVIEW_STAGE
    ...
    if not voters:
        raise RuntimeError(
            ...
            "`avs review-gate` after fixing the charters"
        )
    if state.get("mode") == "fast":
    ...
    from ai_venture_studio.tools import ToolReport
...
def leader_node(state: ReviewState, *, provider_override: str | None) -> dict[str, Any]:
    ...
    from ai_venture_studio.policy import Policy
...
def escalate_node(state: ReviewState) -> dict[str, Any]:
    ...
    resume_hint = f"avs resume {state['review_id']} --decision ack"
...
def _deploy_files(state: ReviewState) -> list[str]:
    ...
    from ai_venture_studio.deploy import detect_deploy_files
